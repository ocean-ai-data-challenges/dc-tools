...
def parse_args():
    ...
    parser.add_argument(
        "--source", type=str, required=True, help="Name of the source to process (e.g., glorys)"
    )
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Directory to store intermediate files"
    )
    parser.add_argument(
...
def main():
    ...
    if config_path:
        ...
        # Fallback: treat parsed args as the config mapping
    ...
    manager = MultiSourceDatasetManager(
        dataset_processor=dataset_processor,
        target_dimensions=get_target_dimensions(config),
